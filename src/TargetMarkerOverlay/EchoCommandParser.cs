...
namespace TargetMarkerOverlay
{
    internal static class EchoCommandParser
    {
        public static bool TryGetMessage(string line, out string message)
        {
            ...
            if (TryGetPipeMessage(line, out message)) return true;
        ...
        private static bool TryGetPipeMessage(string line, out string message)
        {
            ...
            {
                if (!string.Equals(fields[index].Trim(), "0038", StringComparison.OrdinalIgnoreCase)) continue;
                if (index >= 2 && !fields[index - 2].Trim().EndsWith("00", StringComparison.Ordinal)) continue;
                if (index + 2 >= fields.Length) continue;
        ...
        private static bool TryGetColonMessage(string line, out string message)
        {
            ...
            if (lastColon >= 0)
            {
                ...
                if ((possibleHash.Length == 8 || possibleHash.Length == 16) && possibleHash.All(Uri.IsHexDigit))
